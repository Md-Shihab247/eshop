...
const ProductLayout = ({springDisAmount ,springHover ,springDiscount,limited,padding,src="images/product-img.png", catagory="it's missing",tittle="Title missing",price="price is not define",oldPrice,rating,bordering,discount,stack,starckAmount}) => {

    let [ratings,setRatings] = useState(new Array(rating).fill(rating))

  return (
    <div className={`${padding && `p-10`} flex flex-col bg-white rounded-[10px] duration-300 group border border-[#CBCBCB] sm:border-transparent ${bordering && `hover:border hover:border-[#CBCBCB]`} ${springHover && `hover:bg-[#EAEAEA]`}`}>
        <div className={`${padding ? `px-0 pt-0` : `px-6 pt-6`} flex justify-center relative`}>
            <img src={src} alt="product img" className=' bg-center bg-contain'/>
            {discount &&
            ...
            {springDiscount &&
              <div className=' absolute top-0 right-0 font-semibold font-poppins text-2xl leading-6 flex items-center justify-center w-[100px] h-[100px] rounded-[50%] bg-[#FF624C] text-white'> {springDisAmount} </div>
            }
        ...
        <div className={`${padding ? `pt-10 pb-0 px-0` : `pt-10 pb-11 px-6`}`}>
            ...
            <p className='text-[#303030] font-poppins text-[19px] font-semibold leading-7.5 mt-4 duration-300 group-hover:text-[#FF624C] group-hover:underline'>{tittle}</p>
